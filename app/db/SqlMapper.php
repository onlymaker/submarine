 ...
 * 数据模型的封装，支持数据表名前缀
 *
 * Customized by Syncxplus from bzfshop:src/protected/Core/Modal/SqlMapper.php
 *
 * */
...
class SqlMapper extends \DB\SQL\Mapper
{
    ...
    public static function getDbEngine()
    {
        if (null == static::$dbEngine) {
            global $f3;
            // 初始化全站数据库
            static::$tablePrefix = $f3->get('MYSQL_TABLE_PREFIX');
            // 获得数据库引擎
            static::$dbEngine = new Engine(
                $f3->get('SAE') ? 'mysql:host=' . SAE_MYSQL_HOST_M . ';port=' . SAE_MYSQL_PORT . ';dbname=' . SAE_MYSQL_DB : $f3->get('MYSQL_DSN'),
                $f3->get('SAE') ? SAE_MYSQL_USER : $f3->get('MYSQL_USER'),
                $f3->get('SAE') ? SAE_MYSQL_PASS : $f3->get('MYSQL_PWD'));
        }
